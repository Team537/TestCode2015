...
PID_ATune::PID_ATune(PIDSource * source, PIDOutput *Output) {
	watchclock.Start();
	input = source;
	output = Output;
...
void PID_ATune::InitializeRuntime(float outputStartValue)
{
	//initialize working variables the first time around
	float refVal = input->PIDGet();
	peakType = 0;
	peakCount = 0;
	justchanged = false;
	absMax = refVal;
	...
	setpoint = refVal;
	running = true;
	outputStart = outputStartValue;
	outputCurrent = outputStart + oStep;
	output->PIDWrite(outputCurrent);
}
